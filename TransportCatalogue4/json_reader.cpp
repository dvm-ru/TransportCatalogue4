        ...
        for (const auto& elem : vec) {
            if (elem.AsMap().count("type"s)) {
                if (elem.AsMap().at("type"s).AsString() == "Stop"s) {
                    requests_.emplace_back(std::make_unique<RequestStop>(detail::BaseStop(elem.AsMap()))); // fill stop
                }
                else if (elem.AsMap().at("type"s).AsString() == "Bus"s) {
                    requests_.emplace_back(std::make_unique<RequestBus>(detail::BaseBus(elem.AsMap()))); // fill bus
                }
            }
        }
    }
